...
## fits suppl. table 
rf <- data.frame(res)
rf$symbol <- rowData(se)$symbol
rf$descr <- rowData(se)$description
rf <- rf[order(rf$padj),]
...
rownames(mat) <- rowData(se)$symbol[match(rownames(mat), rownames(se))]

ann <- data.frame(diet=samplesM$diet,row.names = colnames(mat))
